...
const installedBrowsers = isTravis
  ? ['Firefox']
  : detectBrowsers
...
let karmaFiles = [
  ...
  // WebWorker script.
  {
    pattern: 'test/harness/factories/workerfs_worker.js',
    included: false,
    watched: true,
  },
  // Source map support
...
if (dropbox) {
  ...
  execSync(
    `node ${path.resolve(
      __dirname,
      './build/scripts/get_db_credentials.js'
    )} ${path.resolve(__dirname, './test/fixtures/dropbox/token.json')}`,
    {
      stdio: 'inherit',
    }
  );
