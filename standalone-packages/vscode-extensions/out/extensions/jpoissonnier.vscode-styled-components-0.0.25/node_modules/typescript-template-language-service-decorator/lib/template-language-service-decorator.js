...
var TemplateLanguageServiceProxy = /** @class */ (function () {
    function TemplateLanguageServiceProxy(typescript, sourceHelper, templateStringService, 
    ...
    TemplateLanguageServiceProxy.prototype.tryAdaptGetCodeFixesAtPosition = function () {
        ...
        this.wrap('getCodeFixesAtPosition', function (delegate) { return function (fileName, start, end, errorCodes, options, preferences) {
            var templateActions = [];
            for (var _i = 0, _a = _this.sourceHelper.getAllTemplates(fileName); _i < _a.length; _i++) {
                var template = _a[_i];
                var nodeStart = template.node.getStart() + 1;
            ...
            return delegate(fileName, start, end, errorCodes, options, preferences).concat(templateActions);
        }; });
    };
    ...
    TemplateLanguageServiceProxy.prototype.tryAdaptGetSignatureHelpItemsAtPosition = function () {
        var _this = this;
        if (!this.templateStringService.getSignatureHelpItemsAtPosition) {
            return;
        }
        this.wrap('getSignatureHelpItems', function (delegate) { return function (fileName, position) {
            var rest = [];
            for (var _i = 2; _i < arguments.length; _i++) {
                rest[_i - 2] = arguments[_i];
            }
            ...
            var signatureHelp = _this.templateStringService.getSignatureHelpItemsAtPosition(context, _this.sourceHelper.getRelativePosition(context, position));
            return signatureHelp ? _this.translateSignatureHelpItems(context, signatureHelp) : undefined;
        }; });
    };
    TemplateLanguageServiceProxy.prototype.tryAdaptGetOutliningSpans = function () {
        var _this = this;
        if (!this.templateStringService.getOutliningSpans) {
            return;
        }
        this.wrap('getOutliningSpans', function (delegate) { return function (fileName) {
            var templateSpans = [];
            for (var _i = 0, _a = _this.sourceHelper.getAllTemplates(fileName); _i < _a.length; _i++) {
                var template = _a[_i];
                for (var _b = 0, _c = _this.templateStringService.getOutliningSpans(template); _b < _c.length; _b++) {
                    var span = _c[_b];
                    templateSpans.push(_this.translateOutliningSpan(template, span));
                }
            }
            return delegate(fileName).concat(templateSpans);
        }; });
    };
    ...
    TemplateLanguageServiceProxy.prototype.adaptDiagnosticsCall = function (delegate, implementation, fileName) {
        ...
        for (var _i = 0, _a = this.sourceHelper.getAllTemplates(fileName); _i < _a.length; _i++) {
            var context_1 = _a[_i];
            for (var _b = 0, _c = implementation(context_1); _b < _c.length; _b++) {
                var diagnostic = _c[_b];
                templateDiagnostics.push(__assign({}, diagnostic, { start: context_1.node.getStart() + 1 + (diagnostic.start || 0) }));
            }
        }
        return baseDiagnostics.concat(templateDiagnostics);
    };
    TemplateLanguageServiceProxy.prototype.translateTextChange = function (context, textChange) {
        return __assign({}, textChange, { span: this.translateTextSpan(context, textChange.span) });
    };
    ...
    TemplateLanguageServiceProxy.prototype.translateCodeAction = function (context, action) {
        var _this = this;
        return __assign({}, action, { fixName: action.fixName || '', changes: action.changes.map(function (change) { return _this.translateFileTextChange(context, change); }) });
    };
    TemplateLanguageServiceProxy.prototype.translateSignatureHelpItems = function (context, signatureHelp) {
        return __assign({}, signatureHelp, { applicableSpan: this.translateTextSpan(context, signatureHelp.applicableSpan) });
    };
    TemplateLanguageServiceProxy.prototype.translateOutliningSpan = function (context, span) {
        return __assign({}, span, { textSpan: this.translateTextSpan(context, span.textSpan), hintSpan: this.translateTextSpan(context, span.hintSpan) });
    };
    TemplateLanguageServiceProxy.prototype.translateTextSpan = function (context, span) {
        return {
