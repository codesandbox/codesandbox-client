...
suite('typescript.previewer', () => {
    test('Should ignore hyphens after a param tag', async () => {
        assert.strictEqual((0, previewer_1.tagsMarkdownPreview)([
            {
                name: 'param',
                text: 'a - b'
            }
    ...
    test('Should parse url jsdoc @link', async () => {
        assert.strictEqual((0, previewer_1.markdownDocumentation)('x {@link http://www.example.com/foo} y {@link https://api.jquery.com/bind/#bind-eventType-eventData-handler} z', [], noopToResource).value, 'x [http://www.example.com/foo](http://www.example.com/foo) y [https://api.jquery.com/bind/#bind-eventType-eventData-handler](https://api.jquery.com/bind/#bind-eventType-eventData-handler) z');
    });
    test('Should parse url jsdoc @link with text', async () => {
        assert.strictEqual((0, previewer_1.markdownDocumentation)('x {@link http://www.example.com/foo abc xyz} y {@link http://www.example.com/bar|b a z} z', [], noopToResource).value, 'x [abc xyz](http://www.example.com/foo) y [b a z](http://www.example.com/bar) z');
    });
    test('Should treat @linkcode jsdocs links as monospace', async () => {
        assert.strictEqual((0, previewer_1.markdownDocumentation)('x {@linkcode http://www.example.com/foo} y {@linkplain http://www.example.com/bar} z', [], noopToResource).value, 'x [`http://www.example.com/foo`](http://www.example.com/foo) y [http://www.example.com/bar](http://www.example.com/bar) z');
    });
    ...
    test('Should ignore unclosed jsdocs @link', async () => {
        assert.strictEqual((0, previewer_1.markdownDocumentation)('x {@link http://www.example.com/foo y {@link http://www.example.com/bar bar} z', [], noopToResource).value, 'x {@link http://www.example.com/foo y [bar](http://www.example.com/bar) z');
    });
    ...
    test('Should render @linkcode symbol name as code', async () => {
        assert.strictEqual((0, previewer_1.plainWithLinks)([
            { "text": "a ", "kind": "text" },
