...
class _TreeBuilder {
    ...
    _consumeText(token) {
        ...
        if (text.length > 0 && text[0] == '\n') {
            ...
            if (parent != null &&
                ...
                this.getTagDefinition(parent.name, this.options.ignoreFirstLf, this.options.selfClosingElements).ignoreFirstLf) {
        ...
        if (this._peek.type === lex.TokenType.TAG_OPEN_END_VOID) {
            ...
            if (!(tagDef.canSelfClose ||
                ...
                (this.options.selfClosingCustomElements &&
                    html_tags_1.isKnownHTMLTag(nameAndNsInfo.fullName) === false))) {
    ...
    _consumeEndTag(endTagToken) {
        ...
        if (this.getTagDefinition(nameInfo.fullName, this.options.ignoreFirstLf, this.options.selfClosingElements).isVoid) {
    ...
    _popElement(fullName) {
        for (let stackIndex = this._elementStack.length - 1; stackIndex >= 0; stackIndex--) {
            ...
            if (!this.getTagDefinition(el.name, this.options.ignoreFirstLf, this.options.selfClosingElements).closedByParent) {
                return false;
            }
        }
        return false;
    ...
    _getElementNameAndNsInfo(prefix, localName, parentElement) {
        ...
        if (prefix == null) {
            prefix = this.getTagDefinition(localName, this.options.ignoreFirstLf, this.options.selfClosingElements).implicitNamespacePrefix;
            if (prefix) {
                implicitNs = true;
            }
