...
var CodeSandboxWindowsService = /** @class */ (function () {
    ...
    CodeSandboxWindowsService.prototype.showSaveDialog = function (windowId, options) {
        ...
        var standardFilename = '/Untitled';
        if (filters && filters[0] && filters[0].extensions && filters[0].extensions[0]) {
            standardFilename = '/Untitled.' + filters[0].extensions[0];
        }
        var pathToSave = window.prompt("Where do you want to save the file?", options.defaultPath + standardFilename);
        return TPromise.as(pathToSave);
    };
    ...
    CodeSandboxWindowsService.prototype.isFocused = function (windowId) {
        return TPromise.as(document.hasFocus());
    };
