...
import { normalize, toFileChangesEvent, } from '../workbench/services/files/node/watcher/common.js';
import { ResourceMap } from '../base/common/map.js';
import { ThrottledDelayer } from '../base/common/async.js';
import { ILifecycleService, } from '../platform/lifecycle/common/lifecycle.js';
import { detectEncodingFromBuffer } from './shims/encoding.js';
...
var CodeSandboxFileService = /** @class */ (function (_super) {
    ...
    CodeSandboxFileService.prototype.resolveFiles = function (toResolve) {
        return TPromise.join(toResolve.map(function (_a) {
            var resource = _a.resource;
            return doFsStat(resource);
        }))
            .then(function (results) { return results.map(function (r) { return ({ success: true, stat: r }); }); });
    };
        ...
        return new TPromise(function (resolve, reject) {
            fs.stat(resource.fsPath, function (err, stat) {
                ...
                fs.readFile(resource.fsPath, function (err, data) {
                    // @ts-ignore
                    var seemsBinary = detectEncodingFromBuffer({ buffer: data, bytesRead: data.length }, options.autoGuessEncoding).seemsBinary;
                    if (seemsBinary && options.acceptTextOnly) {
                        reject(new FileOperationError("Binary", 0 /* FILE_IS_BINARY */, options));
                        return;
                    }
                    var encoding = seemsBinary ? 'base64' : 'utf8';
                    resolve({
                        encoding: encoding,
    ...
    CodeSandboxFileService.prototype.updateContent = function (resource, value, options) {
        ...
        if (resource.fsPath.indexOf('/sandbox/') !== 0) {
            // @ts-ignore
    ...
    CodeSandboxFileService.prototype.moveFile = function (source, target, overwrite) {
        // TODO: make this really rename, now it's just used after we rename inside
        // CodeSandbox
        var module = this.codesandboxService.getFilesByPath()[target.fsPath];
        return TPromise.as((createFileFromModule(target.fsPath, module)));
    };
    ...
    CodeSandboxFileService.prototype.createFile = function (resource, content, options) {
        var module = this.codesandboxService.getFilesByPath()[resource.fsPath];
        if (module && !options.overwrite) {
            return TPromise.wrapError(new FileOperationError("File to create already exists (" + resource.toString() + ")", 4 /* FILE_MODIFIED_SINCE */, options));
        }
    ...
    CodeSandboxFileService.FS_EVENT_DELAY = 500; // aggregate and only emit events when changes have stopped for this duration (in ms)
