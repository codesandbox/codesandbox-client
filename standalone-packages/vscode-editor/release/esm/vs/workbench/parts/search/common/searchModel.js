...
import { ISearchService, resultIsMatch, OneLineRange } from '../../../../platform/search/common/search.js';
import { ITelemetryService } from '../../../../platform/telemetry/common/telemetry.js';
import { overviewRulerFindMatchForeground } from '../../../../platform/theme/common/colorRegistry.js';
import { themeColorFromId } from '../../../../platform/theme/common/themeService.js';
import { IReplaceService } from './replace.js';
import { editorMatchesToTextSearchResults } from '../../../services/search/common/searchHelpers.js';
var Match = /** @class */ (function () {
    function Match(_parent, _result) {
        ...
        this._range = new Range(_result.ranges.startLineNumber + 1, _result.ranges.startColumn + 1, _result.ranges.endLineNumber + 1, _result.ranges.endColumn + 1);
        this._rangeInPreviewText = new Range(_result.preview.matches.startLineNumber + 1, _result.preview.matches.startColumn + 1, _result.preview.matches.endLineNumber + 1, _result.preview.matches.endColumn + 1);
...
var FileMatch = /** @class */ (function (_super) {
    ...
    FileMatch.prototype.updateMatches = function (matches, modelChange) {
        var _this = this;
        var textSearchResults = editorMatchesToTextSearchResults(matches, this._model, this._previewOptions);
    ...
    FolderMatch.prototype.add = function (raw, silent) {
        ...
        raw.forEach(function (rawFileMatch) {
            if (_this._fileMatches.has(rawFileMatch.resource)) {
                var existingFileMatch_1 = _this._fileMatches.get(rawFileMatch.resource);
                rawFileMatch
                    .results
                    .filter(resultIsMatch)
                    .forEach(function (m) {
                    textSearchResultToMatches(m, existingFileMatch_1)
                        .forEach(function (m) { return existingFileMatch_1.add(m); });
                });
                updated.push(existingFileMatch_1);
            }
            else {
...
function textSearchResultToMatches(rawMatch, fileMatch) {
    if (Array.isArray(rawMatch.ranges)) {
        var previewLines_1 = rawMatch.preview.text.split('\n');
        return rawMatch.ranges.map(function (r, i) {
            var previewRange = rawMatch.preview.matches[i];
            var matchText = previewLines_1[previewRange.startLineNumber];
            var adjustedEndCol = previewRange.startLineNumber === previewRange.endLineNumber ?
                previewRange.endColumn :
                matchText.length;
            var adjustedRange = new OneLineRange(0, previewRange.startColumn, adjustedEndCol);
        ...
        var firstNewlineIdx = rawMatch.preview.text.indexOf('\n');
        var matchText = firstNewlineIdx >= 0 ?
        ...
        var match = new Match(fileMatch, adjustedMatch);
        return [match];
    }
}
