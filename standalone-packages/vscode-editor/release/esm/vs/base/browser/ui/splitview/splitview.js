...
import { mapEvent, Emitter } from '../../../common/event.js';
...
var SplitView = /** @class */ (function (_super) {
    ...
    SplitView.prototype.resize = function (index, delta, sizes, lowPriorityIndexes, highPriorityIndexes, overloadMinDelta, overloadMaxDelta) {
        ...
        var minDeltaUp = upIndexes.reduce(function (r, i) { return r + ((typeof _this.viewItems[i].view.snapSize === 'number' ? 0 : _this.viewItems[i].view.minimumSize) - sizes[i]); }, 0);
        var maxDeltaUp = upIndexes.reduce(function (r, i) { return r + (_this.viewItems[i].view.maximumSize - sizes[i]); }, 0);
        var maxDeltaDown = downIndexes.length === 0 ? Number.POSITIVE_INFINITY : downIndexes.reduce(function (r, i) { return r + (sizes[i] - (typeof _this.viewItems[i].view.snapSize === 'number' ? 0 : _this.viewItems[i].view.minimumSize)); }, 0);
        ...
        var tentativeDelta = clamp(delta, minDelta, maxDelta);
        var actualDelta = 0;
        for (var i = 0, deltaUp = tentativeDelta; i < upItems.length; i++) {
            var item = upItems[i];
            var size = SplitView.clamp(item, upSizes[i] + deltaUp /* , upIndexes[i] === index */);
            var viewDelta = size - upSizes[i];
            actualDelta += viewDelta;
            deltaUp -= viewDelta;
            item.size = size;
        }
        for (var i = 0, deltaDown = actualDelta; i < downItems.length; i++) {
            var item = downItems[i];
            var size = SplitView.clamp(item, downSizes[i] - deltaDown);
    ...
    SplitView.clamp = function (item, size) {
        var result = clamp(size, item.view.minimumSize, item.view.maximumSize);
        if (typeof item.view.snapSize !== 'number' || size >= item.view.minimumSize) {
            return result;
        }
        var snapSize = Math.min(item.view.snapSize, item.view.minimumSize);
    ...
    SplitView.prototype.distributeEmptySpace = function () {
        ...
        for (var i = this.viewItems.length - 1; emptyDelta !== 0 && i >= 0; i--) {
            var item = this.viewItems[i];
            var size = SplitView.clamp(item, item.size + emptyDelta);
