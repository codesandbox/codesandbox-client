...
import { nfcall } from '../common/async.js';
import { normalizeNFC } from '../common/normalization.js';
import * as platform from '../common/platform.js';
import * as strings from '../common/strings.js';
import * as uuid from '../common/uuid.js';
import { encode, encodeStream } from './encoding.js';
import * as flow from './flow.js';
import { toDisposable, Disposable } from '../common/lifecycle.js';
...
export function copy(source, target, callback, copiedSourcesIn) {
...
function doWriteFileAndFlush(path, data, options, callback) {
    ...
    fs.open(path, typeof options.flag === 'string' ? options.flag : 'r', options.mode, function (openError, fd) {
        ...
        // CODESANDBOX-EDIT: make fd (first arg) path, since BrowserFS doesn't support file descriptors
        fs.writeFile(path, data, function (writeError) {
            if (writeError) {
        ...
        var interval_1 = setInterval(function () {
            if (isDirectory_1) {
                fs.readdir(path, function (err, dirs) {
                    ...
                    if (!firstRun_1 && lastData_1) {
                        Object.keys(lastData_1).forEach(function (d) {
                            if (dirs.indexOf(d) === -1) {
                                onChange('rename', d);
                                delete lastData_1[d];
                            }
                    ...
                    dirs.forEach(function (d) {
                        ...
                        if (!firstRun_1 && lastData_1) {
                            ...
                            else if (lastData_1[d].mtime.getTime() !== stat.mtime.getTime()) {
                                onChange('change', d);
                                changed = true;
                            }
                        }
                        if (firstRun_1 || changed) {
                            lastData_1[d] = stat;
                        }
                    });
                });
                ...
                try {
                    ...
                    if (lastData_1 && lastData_1.mtime.getTime() !== stat_1.mtime.getTime()) {
                        onChange('change', basename);
                    }
                    lastData_1 = stat_1;
                }
                catch (e) {
                    onChange('rename', basename);
