...
export default (
  ...
  const foundDirectoryId = splitPath.reduce((dirId: ?string, pathPart: string, i: number) => {
    ...
    const nextDirectory = directoriesInDirectory.find(d => d.title === pathPart);
  ...
  const foundModule = modulesInFoundDirectory.find(m => m.title === lastPath);
  if (foundModule) return foundModule;

  // eslint-disable-next-line eqeqeq
  const directoriesInFoundDirectory = directories.filter(m => m.directoryId == foundDirectoryId);
  const foundDirectory = directoriesInFoundDirectory.find(m => m.title === lastPath);

  if (foundDirectory) {
    // eslint-disable-next-line eqeqeq
    const indexModule = modules.find(m => m.directoryId == foundDirectory.id && m.title === 'index');
