...
>>>>>>> more fixes
    ...
    handleMessage = (data) => {
        if (data.action === 'show-correction') {
            ...
            const newMessages = [ ...(this.state.corrections[path] || []), { type: 'warn', message: data.message } ];
            ...
            const newMessages = [ ...(this.state.corrections[path] || []), { type: 'error', message: data.message } ];
    ...
    render() {
        ...
        const files = Object.keys(this.state.corrections).sort().filter((x) => x !== 'root');
