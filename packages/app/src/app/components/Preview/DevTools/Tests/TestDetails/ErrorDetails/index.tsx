...
import { TestError } from 'app/components/Preview/DevTools/Tests/types';
...
const formatDiffMessage = (error: TestError, path: string) => {
    ...
    if (error.matcherResult) {
        finalMessage = `<span style="color:rgba(255, 255, 255, 0.5);">${escapeHtml(error.message)
            ...
            .replace(/^(\+.*)/gm, `<span style="color:${theme.green()}">$1</span>`)}</span>`;
    } else {
        finalMessage = error.message + '\n\n' + error.stack;
    }

    if (error.mappedErrors && error.mappedErrors[0] && error.mappedErrors[0].fileName.endsWith(path)) {
        const mappedError = error.mappedErrors[0];

        const widestNumber =
            Math.max(...mappedError._originalScriptCode.map((code) => (code.lineNumber + '').length)) + 2;
        const margin = Array.from({ length: widestNumber }).map(() => ' ');

        finalMessage += '<br />';
        finalMessage += '<br />';
        finalMessage += '<div>';
        mappedError._originalScriptCode.filter((x) => x.content.trim()).forEach((code) => {
            ...
            finalMessage +=
                ...
                (code.highlight ? `<span style="color:${theme.red()};">></span> ` : '') +
                newMargin.join('') +
                code.lineNumber +
                ' | ' +
