...
class DirectoryEntry extends React.Component {
    constructor(props) {
        ...
        const currentModuleParents = getModuleParents(modules, directories, currentModuleId);
    ...
    componentWillReceiveProps(nextProps, nextState) {
        if (!nextState.open && this.props.store.editor.currentModule !== nextProps.store.editor.currentModule) {
    ...
    getChildren = () => {
        ...
        return [
            ...this.props.store.editor.currentSandbox.modules.filter((m) => m.directoryShortid === shortid),
            ...this.props.store.editor.currentSandbox.directories.filter((d) => d.directoryShortid === shortid)
        ];
    };
    ...
    render() {
        ...
        const title = root ? 'Project' : currentSandbox.directories.find((m) => m.id === id).title;
            ...
            <div style={{ position: 'relative' }}>
                ...
                {!root && (
                    <EntryContainer>
                        ...
                        />
                        <Modal isOpen={this.state.showDeleteDirectoryModal} onClose={this.closeModals} width={400}>
                            <Alert
                ...
                <Opener open={open}>
                    ...
                    />
                    <Modal isOpen={this.state.showDeleteModuleModal} onClose={this.closeModals} width={400}>
                        <Alert
                            title="Delete File"
                            body={
                                <span>
                                    Are you sure you want to delete <b>{this.state.moduleToDeleteTitle}</b>?
                                    <br />
...
export default inject('signals', 'store')(DropTarget('ENTRY', entryTarget, collectTarget)(observer(DirectoryEntry)));
