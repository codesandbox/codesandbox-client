...
export const getContainerStyles = (props) => {
    const { theme } = props;
    const color = props.color || (props.alternative ? theme.primary : theme.templateColor || theme.secondary);
    let styles = `
    ...
    font-size: 14px;
    padding: 0.4rem;
    padding-left: ${props.depth != null ? `calc(${props.depth + 1}rem - 2px)` : 'calc(1rem - 2px)'};
    padding-right: 3rem;
