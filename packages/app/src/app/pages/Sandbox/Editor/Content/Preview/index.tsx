        ...
        if (this.state.aligned) {
            if (width !== this.props.width || height !== this.props.height) {
                if (this.state.aligned === 'bottom') {
                    this.props.signals.editor.setPreviewBounds(this.getBottomCoordinates(props));
                } else {
                    this.props.signals.editor.setPreviewBounds(this.getRightCoordinates(props));
                }
            }
        } else if (width && height) {
            let newWidth = props.store.editor.previewWindow.width;
            if (width - 16 < props.store.editor.previewWindow.width - props.store.editor.previewWindow.x) {
                newWidth = Math.max(64, width - 16 + props.store.editor.previewWindow.x);
            }

            let newHeight = props.store.editor.previewWindow.height;
            if (height - 16 < props.store.editor.previewWindow.height + props.store.editor.previewWindow.y) {
                newHeight = Math.max(64, height - 16 - props.store.editor.previewWindow.y);
            }
