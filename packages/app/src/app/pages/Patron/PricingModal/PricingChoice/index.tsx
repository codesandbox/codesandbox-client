...
export default connect<Props>()
  ...
  .to(
    function PricingChoice({ error, userName, isPatron, isLoggedIn, subscription, price, isUpdatingSubscription, signals, badge }) {
      return (
        <Container>
          <Centered horizontal vertical={false}>
            ...
            <Relative>
              ...
              <PriceInput
                onChange={event =>
                  signals.priceChanged({ price: Number(event.target.value) })
                }
                value={price}
                type="number"
              />
            ...
            <RangeContainer>
              <Range
                onChange={value =>
                  signals.priceChanged({ price: Number(value) })
                }
                min={5}
                max={50}
                step={1}
            ...
            {isLoggedIn ? ( // eslint-disable-line no-nested-ternary
             isPatron ? (
                ...
                <Centered style={{ marginTop: '2rem' }} horizontal>
                  <SubscribeForm
                    subscribe={token =>
                      signals.createSubscriptionClicked({ token })
                    }
