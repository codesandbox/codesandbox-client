...
class Sandboxes extends React.Component<Props> {
    ...
    fetch(force = false) {
        ...
        if (force || !store.profile[source] || !store.profile[source].get(String(page))) {
            switch (source) {
        ...
        return (
            <div>
                {isProfileCurrentUser && (
                    <Notice>
                        You{"'"}re viewing your own profile, so you can see your private and unlisted sandboxes. Others
                        can{"'"}t.
                    </Notice>
                )}
                <SandboxList
