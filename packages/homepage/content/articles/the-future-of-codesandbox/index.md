...
[BTMPL's profile](https://codesandbox.stream/u/BTMPL) contains some great examples,
...
[`react-router`](https://codesandbox.stream/s/XoBNWnBWv). These sandboxes are often
...
replace `github.com` with `codesandbox.stream/s/github` and that path will point to
