...
type Props = {
  sandbox: Pick<Sandbox, 'forkCount' | 'likeCount' | 'viewCount'>;
};
export const Stats: FunctionComponent<Props> = ({
  sandbox: { forkCount, likeCount, viewCount },
}) => (
  <Stack gap={4}>
    <Stat count={likeCount} Icon={HeartIcon} />

    <Stat count={viewCount} Icon={ViewIcon} />

    <Stat count={forkCount} Icon={ForkIcon} />
  </Stack>
);
